...
```
*UKBBcleanR* depends on the *tidyverse* and *data.table* packages. 
```{r eval=FALSE}
install.packages("tidyverse")
install.packages("data.table")
```

Following installation, attach the *UKBBcleanR* and required packages with:
```{r eval=FALSE}
library(UKBBcleanR)
library(tidyverse)
library(data.table)
...
### Available Data
Within *UKBBcleanR*, we include simulated UK Biobank sample data containing 30 individuals and 490 variables. A detailed list of required variables are provided in the `README_required_variables.txt` file. The raw UK Biobank data can be specified in two ways:

* First option (as demonstrated below). Data can be loaded directly into the `tte` function. The user needs to create a data frame containing all specified variables

* Second option. The user creates individual `rds` files that contain the variables specified in the `README_required_variables.txt` file. The user needs to specify the working directory using `setwd()` outside the function where these files are stored. The files are required to be named as: 
    + date_enroll.rds
    + cancer_reg.rds
    + inpatient_data.rds
...
loadedPackages <- c("UKBBcleanR", "tidyverse", "data.table")
invisible(lapply(loadedPackages, require, character.only = TRUE))

# Create data frame containing sample data
test_data<- as.data.frame(combined_data)
```

### Set function parameters
For this example data cleaning, we will use CLL (ICD-10 code "C911") as the outcome of interest. We will also identify other non-CLL cancers in the data to control in our analyses. 
```{r eval=TRUE}
...
Data creation is performed using `tte`. Within this example, we will not remove anyone identified to have prevalent cancer and the time of enrollment. 
```{r eval=TRUE}
...
The individual with prevalent CLL has now been removed in the `tte_output_2` analysis. The above variables can be used directly in any time-to-event analyses. 
